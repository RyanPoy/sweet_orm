...
class TestSQLiteRecordsetUpdate(unittest.TestCase):
    ...
    def test_update(self):
        ...
        tb = SQLiteRecordset(db=db, tbname='users').where(age__gte=40).or_where(name="Ryan")
    ...
    def test_update_with_join(self):
        db = self.get_db()
        db.execute_rowcount = mock.MagicMock(return_value=3)
        tb = SQLiteRecordset(db=db, tbname='users').where(users__id=[1,2,3]).or_where(users__name="Ryan").join('cars', "users.id=cars.user_id")
